        ...
        return paymentSummaryRows
            .map((row, index) => {
                const style = [
                    ...
                    index === 2 ? borderTop : "",
                    index === paymentSummaryRows.length - 1 ? "padding-bottom:10px; line-height:1.4;" : "",
                ].join(" ");
...
export const buildOutsideDispenseInfoSectionHtml = ({
    customer,
    invoice,
    generatedBy,
    invoiceNoRowHtmlPdf,
  ...
  <div style="display:flex; border:1px solid #000; margin:10px 0; font-size:13px;">
    <div style="width:65%; background:#fafafa; padding:12px; border-right:1px solid #000;">
      <table style="width:100%; font-size:12px;">
        ...
        <tr><td style="font-weight:bold;">Address</td><td>:</td><td>${escapeHtml(customer.address)}</td></tr>
        <tr><td style="font-weight:bold;">Disease</td><td>:</td><td>${escapeHtml(customer.disease)}</td></tr>
        <tr><td style="font-weight:bold;">Generated By</td><td>:</td><td>${escapeHtml(generatedBy)}</td></tr>
      </table>
    </div>
    <div style="width:35%; background:#fafafa;">
      <div style="text-align:center; font-weight:bold; font-size:17px; border-bottom:1px solid #000; padding:8px; background:#f5f0eb;">
        OUTSIDE DISPENSE
      </div>
...
export const buildOutsideDispenseMedicinesSectionHtml = ({ medicinesRows }) => sectionWrap(`
  <table style="width:100%; border-collapse:collapse; margin:8px 0 0; font-size:11px;">
    <thead>
...
export const buildOutsideDispenseSummarySectionHtml = ({
    amountInWords,
    paymentHistoryHtmlPdf,
    paymentSummaryHtmlPdf,
}) => sectionWrap(`
  <div style="display:flex; margin-top:12px; padding:0 0 12px;">
    <div style="width:55%; padding-right:15px;">
      <div style="font-weight:bold; font-size:13px; margin-bottom:6px;">Amount in Words:</div>
      <div style="font-style:italic; font-size:12px; line-height:1.5;">${escapeHtml(amountInWords)}</div>
      ${paymentHistoryHtmlPdf}
    </div>
