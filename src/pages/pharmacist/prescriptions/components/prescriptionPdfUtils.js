...
export const formatPatientAge = (age, ageUnit = "years") => {
    if (age === "" || age == null) return "";
    const num = Number(age);
    if (!Number.isFinite(num)) return String(age);
    const unit = ageUnit === "months" ? (num === 1 ? "month" : "months") : num === 1 ? "year" : "years";
    return `${num} ${unit}`;
};
        ...
        <tr>
          <td style="width:65%; background:#f9f5f0; padding:12px; border-right:1px solid #000; vertical-align:top;">
            <table style="width:100%; border-collapse:collapse;">
              ...
              <tr><td style="${labelCell}">Age</td><td style="${colonCell}">:</td><td style="${valueCell}">${escapeHtml(formatPatientAge(patient.age, patient.ageUnit))}</td></tr>
              <tr><td style="${labelCell}">Gender</td><td style="${colonCell}">:</td><td style="${valueCell}">${escapeHtml(patient.gender)}</td></tr>
