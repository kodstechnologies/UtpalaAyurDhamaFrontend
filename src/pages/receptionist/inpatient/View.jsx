...
function Inpatient_View() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (invoiceIds.length > 0) {
          try {
            const invoicesResponse = await axios.get(getApiUrl("invoices"), {
              headers: getAuthHeaders(),
              params: {
                page: 1,
            ...
            if (invoicesResponse.data.success && invoicesResponse.data.data) {
              ...
              invoicesResponse.data.data.forEach((invoice) => {
                if (invoice.inpatient) {
                  const inpatientId =
                    invoice.inpatient._id || invoice.inpatient;
                  if (
                    !invoicesMap[inpatientId] ||
                    new Date(invoice.createdAt) >
                      new Date(invoicesMap[inpatientId].createdAt)
                  ) {
        ...
        const transformedInpatients = inpatientsData.map((inpatient) => {
          ...
          const patientProfileId =
            inpatient.patient?._id || inpatient.patient || null;
          ...
          return {
            ...
            doctorName:
              inpatient.doctor?.user?.name ||
              inpatient.examinations?.[0]?.doctor?.user?.name ||
              "N/A",
  ...
  const stats = useMemo(() => {
    return {
      ...
      underObservation: inpatients.filter(
        (p) => p.admitStatus === "Under Observation",
      ).length,
      pendingAllocation: inpatients.filter(
        (p) => p.admitStatus === "Pending Allocation",
      ).length,
      discharged: inpatients.filter((p) => p.admitStatus === "Discharged")
        .length,
    };
                                ...
                                patient.allocatedNurse === "N/A" ? (
                                  ...
                                  <>
                                    <button
                                      ...
                                      className="btn btn-sm"
                                      onClick={() =>
                                        handleOpenAllocationModal(patient)
                                      }
                                      style={{
