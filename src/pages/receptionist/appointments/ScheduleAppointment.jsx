...
function ScheduleAppointmentPage() {
  ...
  const [isLoadingExistingAppointment, setIsLoadingExistingAppointment] =
    useState(false);
  const [hasExistingAppointment, setHasExistingAppointment] = useState(false);
  const [preAssignedDoctorName, setPreAssignedDoctorName] = useState("");

  // Fetch reception patient record to check if a doctor is already assigned via primaryDoctor
  const fetchExistingAppointment = useCallback(async () => {
    if (!patientId) return;

    setIsLoadingExistingAppointment(true);
    try {
      // Use reception-patients/:id which populates patientProfile.primaryDoctor fully
      const endpoint = isFamilyMember
        ? `family-members/${patientId}/receptionist`
      ...
      if (response.data.success && response.data.data) {
        ...
        const primaryDoctor = record.patientProfile?.primaryDoctor;

        if (primaryDoctor) {
          const doctorId = primaryDoctor._id || primaryDoctor;

          // Build a display name from populated doctor data
          const docName =
            primaryDoctor.user?.name ||
            `${primaryDoctor.firstName || ""} ${primaryDoctor.lastName || ""}`.trim() ||
            "";
          const displayName = primaryDoctor.specialization
            ? `${docName} - ${primaryDoctor.specialization}`
            : docName;

          console.log("Pre-filling form with primary doctor:", {
            doctorId,
            displayName,
          });
          ...
          if (displayName) setPreAssignedDoctorName(displayName);
        }
      }
    } catch (error) {
      console.log(
        "No primary doctor found or error fetching reception patient:",
        error.message,
      );
      // Not an error — patient simply has no doctor assigned yet
    } finally {
      setIsLoadingExistingAppointment(false);
    }
  }, [patientId, isFamilyMember]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isFamilyMember) {
        ...
        if (
          !patientProfileResponse.data.success ||
          !patientProfileResponse.data.data
        ) {
          throw new Error(
            "Patient profile not found for family member. Please ensure the family member is properly registered.",
          );
        }
        ...
        if (!patientProfileId) {
          throw new Error(
            "Patient profile not found. Please ensure the patient is properly registered.",
          );
        }
      }
      ...
      if (appointmentResponse.data.success) {
        ...
        if (isFamilyMember) {
          try {
            ...
            console.warn(
              "Failed to assign doctor to family member:",
              assignError,
            );
            // Don't fail the whole operation if doctor assignment fails
            toast.warning(
              "Appointment scheduled, but failed to assign doctor to family member profile.",
            );
          }
        }
        ...
        throw new Error(
          appointmentResponse.data.message || "Failed to schedule appointment",
        );
      }
    } catch (error) {
  ...
  return (
    <div>
      <HeadingCard
        ...
        subtitle={`Patient: ${patientName}${hasExistingAppointment ? " - Edit Mode" : ""}`}
        breadcrumbItems={[
          ...
          {hasExistingAppointment && (
            ...
            >
              <p style={{ margin: 0, color: "#2e7d32", fontSize: "14px" }}>
                ...
                {preAssignedDoctorName ? (
                  <>
                    {" "}
                    : <strong>{preAssignedDoctorName}</strong>
                  </>
                ) : null}
                . You can select a different doctor or keep the same one.
              </p>
            </Box>
          )}
