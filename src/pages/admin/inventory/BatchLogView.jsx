...
import { Box, Grid, Chip, Typography, alpha, useTheme, CircularProgress } from "@mui/material";
...
function BatchLogView() {
    ...
    const handleViewDetails = (row) => {
        ...
        navigate(`/admin/inventory/batch-log-details?${params.toString()}`);
    };
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <TableComponent
                title="Batch Log"
                subtitle={`${filteredRows.length} batches found`}
                columns={columns}
                rows={filteredRows}
                actions={actions}
