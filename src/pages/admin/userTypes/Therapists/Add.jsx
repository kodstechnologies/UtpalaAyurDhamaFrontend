...
function Add_Therapists() {
  ...
  const handleSave = async () => {
    ...
    if (!emailError && therapist.email) {
      try {
        const checkResult = await adminUserService.checkEmailAvailability(therapist.email, "Therapist");
        if (checkResult.exists) {
          toast.error("This email is already registered. Please use a different email.");
          setErrors((prev) => ({ ...prev, email: "This email is already registered" }));
          return;
    ...
    if (!phoneError && therapist.phone) {
      try {
        const checkResult = await adminUserService.checkPhoneAvailability(therapist.phone, "Therapist");
        if (checkResult.exists) {
          toast.error("This phone number is already registered. Please use a different phone number.");
          setErrors((prev) => ({ ...prev, phone: "This phone number is already registered" }));
          return;
    ...
    try {
      ...
      toast.error(error.message || "Error creating therapist");
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-a)" }}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 p-4 rounded-xl border" style={{
                        ...
                        <div>
                          ...
                          <button
                            type="button"
                            onClick={() => document.getElementById('profile-upload-therapist').click()}
      ...
      <InputDialogModal
        ...
        validate={(value) => {
          if (therapist.languages.includes(value)) {
      ...
      <InputDialogModal
        ...
        label="Certification"
        placeholder="e.g., Manual Therapy, Sports Rehabilitation"
        confirmText="Add Certification"
        validate={(value) => {
          if (therapist.certifications.includes(value)) {
