...
function Add_Receptionists() {
  ...
  const handleSave = async () => {
    ...
    if (!emailError && receptionist.email) {
      try {
        const checkResult = await adminUserService.checkEmailAvailability(receptionist.email, "Receptionist");
        if (checkResult.exists) {
          toast.error("This email is already registered. Please use a different email.");
          setErrors((prev) => ({ ...prev, email: "This email is already registered" }));
          return;
    ...
    if (!phoneError && receptionist.phone) {
      try {
        const checkResult = await adminUserService.checkPhoneAvailability(receptionist.phone, "Receptionist");
        if (checkResult.exists) {
          toast.error("This phone number is already registered. Please use a different phone number.");
          setErrors((prev) => ({ ...prev, phone: "This phone number is already registered" }));
          return;
    ...
    try {
      ...
      toast.error(error.message || "Error creating receptionist");
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-a)" }}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 p-4 rounded-xl border" style={{
                        ...
                        <div>
                          <input
                            type="file"
                            id="profile-upload-receptionist"
                            className="hidden"
                            accept="image/*"
                          ...
                          <button
                            type="button"
                            onClick={() => document.getElementById('profile-upload-receptionist').click()}
      ...
      <InputDialogModal
        ...
        validate={(value) => {
          if (receptionist.languages.includes(value)) {
      ...
      <InputDialogModal
        ...
        title="Add Skill"
        label="Skill"
        placeholder="e.g., Patient Registration, Billing"
        confirmText="Add Skill"
        validate={(value) => {
          if (receptionist.skills.includes(value)) {
