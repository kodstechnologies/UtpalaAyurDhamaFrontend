...
function Edit_Nurs() {
    ...
    const checkEmailAvailability = async (email) => {
        ...
        emailCheckTimeoutRef.current = setTimeout(async () => {
            try {
                const checkResult = await adminUserService.checkEmailAvailability(email, "Nurse", nurseId);
    ...
    const checkPhoneAvailability = async (phone) => {
        ...
        phoneCheckTimeoutRef.current = setTimeout(async () => {
            try {
                const checkResult = await adminUserService.checkPhoneAvailability(phone, "Nurse", nurseId);
                if (checkResult.exists) {
    ...
    const handleSave = async () => {
        ...
        if (!emailError && nurse.email) {
            try {
                const checkResult = await adminUserService.checkEmailAvailability(nurse.email, "Nurse", nurseId);
                if (checkResult.exists) {
                    toast.error("This email is already registered. Please use a different email.");
                    setErrors((prev) => ({ ...prev, email: "This email is already registered" }));
                    return;
        ...
        if (!phoneError && nurse.phone) {
            try {
                const checkResult = await adminUserService.checkPhoneAvailability(nurse.phone, "Nurse", nurseId);
                if (checkResult.exists) {
                    toast.error("This phone number is already registered. Please use a different phone number.");
                    setErrors((prev) => ({ ...prev, phone: "This phone number is already registered" }));
                    return;
        ...
        try {
            ...
            const updateData = {
                ...
                dob: nurse.dob ? new Date(nurse.dob).toISOString() : undefined,
                joiningDate: nurse.joiningDate ? new Date(nurse.joiningDate).toISOString() : undefined
            };

            const response = await adminUserService.updateUser("Nurse", nurseId, updateData);
            if (response.success) {
                setShowSuccess(true);
                toast.success("Nurse updated successfully!");
                setTimeout(() => {
                    setShowSuccess(false);
                    navigate(`/admin/nursing/view/${nurseId}`);
                }, 2000);
            } else {
                toast.error(response.message || "Failed to update nurse");
            }
        } catch (error) {
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 p-4 rounded-xl border hover:shadow-sm transition-all duration-200" style={{
                                                ...
                                                <div>
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => document.getElementById('profile-upload-nurse').click()}
...
function FormInput({ label, icon: Icon, type = "text", value, onChange, required = false, placeholder = "", error = "", maxLength, ...props }) {
    return (
        <div className="flex flex-col gap-2">
            ...
            <div
                className="flex items-center gap-3 p-3 rounded-xl transition-all duration-300 ease-in-out hover:shadow-sm focus-within:shadow-md group"
                style={{
                    backgroundColor: "var(--color-bg-card)",
