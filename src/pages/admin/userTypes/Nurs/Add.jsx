...
function Add_Nurs() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        setNurse(prev => ({ ...prev, profilePicture: response.data.url }));
  ...
  const checkEmailAvailability = async (email) => {
    ...
    emailCheckTimeoutRef.current = setTimeout(async () => {
      try {
        const checkResult = await adminUserService.checkEmailAvailability(email, "Nurse");
  ...
  const checkPhoneAvailability = async (phone) => {
    ...
    phoneCheckTimeoutRef.current = setTimeout(async () => {
      try {
        const checkResult = await adminUserService.checkPhoneAvailability(phone, "Nurse");
        if (checkResult.exists) {
  ...
  const handleLanguageConfirm = (language) => {
    if (language && !nurse.languages.includes(language)) {
      setNurse(prev => ({
        ...prev,
        languages: [...prev.languages, language]
      }));
      toast.success("Language added successfully!");
    } else if (nurse.languages.includes(language)) {
  ...
  const handleCertificationConfirm = (certification) => {
    if (certification && !nurse.certifications.includes(certification)) {
      setNurse(prev => ({
        ...prev,
        certifications: [...prev.certifications, certification]
      }));
      toast.success("Certification added successfully!");
    } else if (nurse.certifications.includes(certification)) {
  ...
  const handleSave = async () => {
    ...
    if (!emailError && nurse.email) {
      try {
        const checkResult = await adminUserService.checkEmailAvailability(nurse.email, "Nurse");
        if (checkResult.exists) {
          toast.error("This email is already registered. Please use a different email.");
          setErrors((prev) => ({ ...prev, email: "This email is already registered" }));
          return;
    ...
    if (!phoneError && nurse.phone) {
      try {
        const checkResult = await adminUserService.checkPhoneAvailability(nurse.phone, "Nurse");
        if (checkResult.exists) {
          toast.error("This phone number is already registered. Please use a different phone number.");
          setErrors((prev) => ({ ...prev, phone: "This phone number is already registered" }));
          return;
    ...
    try {
      ...
      const nurseData = {
        ...
        qualifications: nurse.qualifications || undefined,
        shiftAvailability: nurse.shiftAvailability || undefined,
        address: nurse.address || undefined,
        emergencyContact: nurse.emergencyContact || undefined,
        languages: nurse.languages && nurse.languages.length > 0 ? nurse.languages : undefined,
        certifications: nurse.certifications && nurse.certifications.length > 0 ? nurse.certifications : undefined,
        bio: nurse.bio || undefined,
        profilePicture: nurse.profilePicture || undefined,
        status: nurse.status || "Active",
      };

      // Remove undefined values
      Object.keys(nurseData).forEach(key => {
        if (nurseData[key] === undefined || nurseData[key] === null || nurseData[key] === "") {
          delete nurseData[key];
        }
      });

      const response = await adminUserService.createUser("Nurse", nurseData);
      ...
      if (response.success) {
        ...
        toast.error(response.message || "Failed to create nurse");
        setIsSaving(false);
      }
    } catch (error) {
      console.error("Error creating nurse:", error);
      toast.error(error.message || "Error creating nurse");
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-a)" }}>
      ...
      <InputDialogModal
        ...
        validate={(value) => {
          if (nurse.languages.includes(value)) {
      ...
      <InputDialogModal
        ...
        validate={(value) => {
          if (nurse.certifications.includes(value)) {
