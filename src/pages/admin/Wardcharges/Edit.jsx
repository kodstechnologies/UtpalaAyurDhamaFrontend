...
function Ward_Charges_Edit() {
    ...
    // Load existing ward charge data
    useEffect(() => {
        const loadData = async () => {
            if (!id) {
                toast.error("Ward charge ID is missing");
                navigate("/admin/ward-charges/view");
                return;
            }

            try {
                setIsLoading(true);
                const response = await wardChargeService.getWardChargeById(id);
                
                if (response.success && response.data) {
                    const charge = response.data;
                    setWardCategory(charge.wardCategory || "");
                    setDailyRate(charge.dailyRate?.toString() || "");
                    setDescription(charge.description || "");
                    setIsActive(charge.isActive ?? true);
                } else {
                    toast.error(response.message || "Failed to fetch ward charge details");
                    navigate("/admin/ward-charges/view");
                }
            } catch (error) {
                console.error("Error fetching ward charge:", error);
                toast.error(error.message || "Failed to fetch ward charge details");
                navigate("/admin/ward-charges/view");
            } finally {
                setIsLoading(false);
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await wardChargeService.updateWardCharge(id, payload);
            
            if (response.success) {
                toast.success("Ward charge updated successfully");
                navigate("/admin/ward-charges/view", { state: { refresh: true } });
            } else {
                toast.error(response.message || "Failed to update ward charge");
            }
        } catch (error) {
            console.error("Error updating ward charge:", error);
            const errorMessage = error.message || error.response?.data?.message || "Failed to update ward charge";
    ...
    return (
        <Box sx={{ p: 3 }}>
            <HeadingCard
                title="Edit Ward Charge"
                subtitle="Update ward charge details"
                breadcrumbItems={[
...
export default Ward_Charges_Edit;
