...
function ExaminationRecordsFormView({ patient, appointmentId, appointmentData, onSubmitSuccess, examinationId, examinationData, isEditMode = false }) {
    ...
    const [formData, setFormData] = useState({
        ...
        urine: "",
        sleep: "",
        // Complaints (Chief Complaints section: diseased, duration, severity - all text)
        diseased: "",
        duration: "",
        severity: "",
        chiefComplaint: "",
        ...
        const fetchData = async () => {
            if (isEditMode && examinationData) {
                try {
                    ...
                    const physicalExam = examinationData.physicalExamination
                        || (examinationData.personalHistory && typeof examinationData.personalHistory === "object"
                            ? examinationData.personalHistory
                            : {});
                    const personalHistoryText = typeof examinationData.personalHistory === "string"
                        ? examinationData.personalHistory
                        : "";
                    ...
                    setFormData({
                        ...
                        associatedComplaint: examinationData.associatedComplaint || "",
                        familyHistory: examinationData.familyHistory || "",
                        personalHistory: personalHistoryText,
                        personalBowel: physicalExam.bowel || "",
                        personalAppetite: physicalExam.appetite || "",
                        personalMicturition: physicalExam.micturition || "",
                        personalSleep: physicalExam.sleep || "",
                        dietAndHydration: physicalExam.dietAndHydration || "",
                        physicalActivity: physicalExam.physicalActivity || "",
                        habits: physicalExam.habits || "",
                        socialHistory: examinationData.socialHistory || "",
                        diseased: examinationData.complaints || "",
    ...
    // Reset isSaved if form data changes after being saved (allows re-saving)
    useEffect(() => {
        if (storageKey && !isSubmitting) {
            try {
                ...
                if (isSaved && savedData) {
                    ...
                    const hasChanges = JSON.stringify(formData) !== JSON.stringify(parsed);
                    if (hasChanges) {
                        // Form has changed, allow saving again
                        setIsSaved(false);
                    }
                }
                // Always update localStorage with current form data
                localStorage.setItem(storageKey, JSON.stringify(formData));
            } catch (error) {
                console.error("Error saving to localStorage:", error);
            }
        }
    }, [formData, storageKey, isSaved, isSubmitting]);
    ...
    const handleSave = async () => {
        ...
        // Reset isSaved when user explicitly clicks save to allow re-saving
        // This allows users to save again if they've made changes or want to update
        if (isSaved) {
            setIsSaved(false);
        }
        ...
        try {
            ...
            // NOTE: Do not include treatments array when creating - treatments should be added separately via API
            const examinationData = {
                ...(appointmentId ? { appointment: appointmentId } : { patient: patient._id }),
                // Chief Complaint (diseased from Chief Complaints section)
                complaints: formData.diseased || formData.chiefComplaint || "",
                associatedComplaint: formData.associatedComplaint || "",
                familyHistory: formData.familyHistory || "",
                personalHistory: formData.personalHistory || "",
                physicalExamination: {
                    bowel: formData.personalBowel || "",
                    appetite: formData.personalAppetite || "",
                    micturition: formData.personalMicturition || "",
                    sleep: formData.personalSleep || "",
                    dietAndHydration: formData.dietAndHydration || "",
                ...
                customFields: [
                    // Diagnosis section: selected Chief Complaint (dropdown) - for display in details
                    formData.chiefComplaint && { label: "Chief Complaint (Diagnosis)", value: formData.chiefComplaint },
            ...
            if (admitPatient) {
                examinationData.inpatientData = {
                    ...
                    reason: inpatientFormData.reason || formData.diseased || formData.chiefComplaint || "Admitted after consultation",
            ...
            if (isEditMode) {
                ...
                let existingExaminationId = null;
                if (appointmentId) {
                    try {
                        const existingExamResponse = await axios.get(
                            getApiUrl(`examinations/by-appointment/${appointmentId}`),
                            { headers: getAuthHeaders() }
                        );
                        if (existingExamResponse.data.success && existingExamResponse.data.data) {
                            existingExaminationId = existingExamResponse.data.data._id;
                        }
                    } catch (error) {
                        // If no examination exists (404), that's fine - we'll create a new one
                        if (error.response?.status !== 404) {
                            console.error("Error checking for existing examination:", error);
                        }
                    }
                }
                ...
                if (existingExaminationId) {
                    ...
                    try {
                        ...
                        if (createError.response?.status === 409) {
                            const errorMsg = createError.response?.data?.message || "An examination already exists. Please update the existing one.";
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                            ...
                            <Grid container spacing={2}>
                                <Grid item xs={12}>
                                    <TextField
                                        ...
                                        onChange={handleChange("diseased")}
                                        disabled={!isEditing}
                                        size="small"
                                    />
                                </Grid>
                        ...
                        <FormSection
                            ...
                            subtitle="Related or secondary complaints"
                            icon={MedicalInformation}
                        >
                            <TextField
                                fullWidth
                                ...
                                onChange={handleChange("associatedComplaint")}
                                disabled={!isEditing}
                                size="small"
                            />
                        </FormSection>
                        ...
                        <FormSection
                            ...
                            subtitle="Hereditary and familial medical conditions"
                            icon={Groups}
                        >
                            <TextField
                                fullWidth
                        ...
                        <FormSection
                            ...
                            subtitle="Occupation, living conditions, and social factors"
                            icon={People}
                        >
                            <TextField
                                fullWidth
                                ...
                                {isImagesLoading ? (
                                    ...
                                    <Grid container spacing={2}>
                                        {uploadedPatientImages.map((image) => (
                                            <Grid item xs={12} sm={6} key={image._id}>
                        ...
                        >
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <TextField
                                        fullWidth
                                        label="Final Prakriti"
                                        variant="outlined"
                                        value={formData.finalPrakriti}
                                        onChange={handleChange("finalPrakriti")}
                                        disabled={!isEditing}
                                        size="small"
                                    />
                                </Grid>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <TextField
                                        ...
                                        onChange={handleChange("otherDiseases")}
                                        disabled={!isEditing}
                                        size="small"
                                        placeholder="Other diseases (if any)"
                                    />
                                </Grid>
                                <Grid item xs={12}>
                                    <TextField
