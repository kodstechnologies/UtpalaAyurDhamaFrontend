...
const getAllInpatients = async (params) => {
  const response = await axios.get(getApiUrl("inpatients"), {
...
const getInpatientById = async (id) => {
  const response = await axios.get(getApiUrl(`inpatients/${id}`), {
    headers: getAuthHeaders(),
  });
  return response.data;
};
...
const createInpatient = async (data) => {
  const response = await axios.post(getApiUrl("inpatients"), data, {
    headers: getAuthHeaders(),
  });
  return response.data;
};
...
const updateInpatient = async (id, data) => {
  const response = await axios.patch(getApiUrl(`inpatients/${id}`), data, {
    headers: getAuthHeaders(),
  });
  return response.data;
};
...
const dischargeInpatient = async (id, data) => {
  ...
  const response = await axios.patch(
    getApiUrl(`inpatients/${id}/discharge`),
    data,
    { headers: getAuthHeaders() },
  );
  return response.data;
...
const getDischargeSummary = async (id) => {
  const response = await axios.get(
    getApiUrl(`inpatients/${id}/discharge-summary`),
    { headers: getAuthHeaders() },
  );
  return response.data;
};

const getUnifiedBillingSummary = async (patientId, inpatientId = null) => {
  const params = inpatientId ? { inpatientId } : {};
  const response = await axios.get(
    getApiUrl(`inpatients/patient/${patientId}/billing`),
...
const getOutpatientBillingSummary = async (patientId, options = {}) => {
  ...
  const response = await axios.get(
    getApiUrl(`inpatients/patient/${patientId}/billing/outpatient`),
    {
      headers: getAuthHeaders(),
...
const finalizeOutpatientBilling = async (patientId, data, options = {}) => {
  ...
  const response = await axios.post(
    getApiUrl(`inpatients/patient/${patientId}/billing/outpatient/finalize`),
    data,
    {
      headers: getAuthHeaders(),
      params: { examinationId },
...
const downloadDischargeReport = async (id) => {
  const response = await axios.get(
    getApiUrl(`inpatients/${id}/discharge-report`),
    {
      headers: getAuthHeaders(),
...
const downloadOutpatientBillingReport = async (
  patientId,
  examinationId = null,
) => {
  const url = getApiUrl(
    `inpatients/patient/${patientId}/billing/outpatient/report`,
  );
...
const getInpatientTimeline = async (id) => {
  const response = await axios.get(getApiUrl(`inpatients/${id}/timeline`), {
    headers: getAuthHeaders(),
  });
  return response.data;
};
